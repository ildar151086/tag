...
void fillTagsElements(int** tags, int sizeMode) {
    ...
    cout << "\t ручным образом - напишите 2\n\t\t" << endl;
    cin >> caseUser;
...
/// <summary>
/// Обнуляем самое максимальное число в двумерном массиве. "Пустая ячейка" для пятнашки
/// </summary>
...
/// <summary>
/// Проверяем последовательность на выигрушную. Все числа должны быть отсортированы по возврастанию
/// </summary>
/// <param name="tags">Двумерный массив</param>
/// <param name="sizeMode">Размер доски пятнашки</param>
/// <returns></returns>
bool winsCombinate(int** tags, int sizeMode) {
    // Берем первое значение массива
    int value = tags[0][0];

    for (int i = 0; i < sizeMode; i++) {
        for (int j = 0; j < sizeMode; j++) {
            if (tags[i][j] >= value && value != CLOSE) {
                // Если каждое следующие число больше предыдушего и не равно пустой ящейки
                value = tags[i][j];
            }
            else
                if (tags[i][j] == CLOSE) {
                    // Если нашли пустую ячейку
                    if (i == sizeMode - 1 && j == sizeMode - 1) {
                        // Проверяем если это последний инднекс массива то массив отсортирован
                        return true;
                    } else {
                        // Пустая ячейка не в конце массива то массив не осортирован
                        return false;
                    }
                } else {
                    // Пустая ячейка не в конце массива то массив не осортирован
...
void prossesGameForHuman(int** tags, int sizeMode, Statistic& statictic) {
    ...
    while (!gameEnd) {
        ...
        if (winsCombinate(tags, sizeMode)) {
        ...
        // Проверяем находиться ли пустая ячейка в соседней ячейки
...
int main() {
    ...
    // Удаление максимальнрого значения из элементов в двумерном массиве для создание пустой ячейки
    ...
    // Реализация логики игры 1) при Ручном  подходе
